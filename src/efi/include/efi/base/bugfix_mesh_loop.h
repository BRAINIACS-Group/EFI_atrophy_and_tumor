 ...
 *
 * This file is part of the deal.II library and has been modified for the
 * EFI library. See also the deal.II copyright agreement (www.dealii.org).
 *
...
#ifndef SRC_EFI_INCLUDE_EFI_BASE_BUGFIX_MESH_LOOP_H_
#define SRC_EFI_INCLUDE_EFI_BASE_BUGFIX_MESH_LOOP_H_
...
namespace efi_internal
{
  ...
  inline bool
  is_active_iterator(const dealii::TriaActiveIterator<ACCESSOR> &)
  {
    return true;
  }
  ...
  inline bool
  is_active_iterator(
    const dealii::FilteredIterator<dealii::TriaActiveIterator<ACCESSOR>> &)
  {
    return true;
  }
  ...
  void
  assemble(const dealii::MeshWorker::DoFInfoBox<dim, DOFINFO> &dinfo, A *assembler)
  {
   ...
   * iterator type.
   *
   * This specialization is for dealii::IteratorRange, which may have either a
   * TriaActiveIterator or a FilteredIterator as its base type.
   */
  template <class CellIteratorType>
  struct CellIteratorBaseType<dealii::IteratorOverIterators<CellIteratorType>>
  {
    /**
     * Type definition for the cell iterator type.
     */
   ...
   */
  template <class CellIteratorType>
  struct CellIteratorBaseType<dealii::FilteredIterator<CellIteratorType>>
  {
    /**
     * Type definition for the cell iterator type.
     */
 ...
 * and separates the assembly of face terms (efi_internal faces, boundary faces,
 ...
 * dealii::MeshWorker::AssembleFlags @p flags that are passed. The @p cell_worker is passed the
 ...
 * subface_index is dealii::numbers::invalid_unsigned_int) in addition to the usual
 * ScratchData and CopyData objects.
 *
 * If the flag dealii::MeshWorker::AssembleFlags::assemble_own_cells is passed, then the default
 ...
 * dealii::MeshWorker::AssembleFlags::cells_after_faces.
 *
 * If the flag dealii::MeshWorker::AssembleFlags::assemble_own_interior_faces_once is specified,
 ...
 * dealii::MeshWorker::AssembleFlags contains only @p assemble_own_cells, and can be used as a
...
template <class CellIteratorType,
          ...
          class CellIteratorBaseType =
            typename efi_internal::CellIteratorBaseType<CellIteratorType>::type>
void
mesh_loop(
  const CellIteratorType &                         begin,
  const typename dealii::identity<CellIteratorType>::type &end,

  const typename dealii::identity<std::function<
    void(const CellIteratorBaseType &, ScratchData &, CopyData &)>>::type
    &cell_worker,
  const typename dealii::identity<std::function<void(const CopyData &)>>::type
    &copier,

  const ScratchData &sample_scratch_data,
  const CopyData &   sample_copy_data,

  const dealii::MeshWorker::AssembleFlags flags = dealii::MeshWorker::assemble_own_cells,

  const typename dealii::identity<std::function<void(const CellIteratorBaseType &,
                                             const unsigned int,
                                             ScratchData &,
                                             CopyData &)>>::type
  ...
  const unsigned int chunk_size   = 8)
{
  ...
  Assert(
    (flags &
     (dealii::MeshWorker::assemble_own_interior_faces_once | dealii::MeshWorker::assemble_own_interior_faces_both)) !=
      (dealii::MeshWorker::assemble_own_interior_faces_once | dealii::MeshWorker::assemble_own_interior_faces_both),
    dealii::ExcMessage(
      "You can only specify assemble_own_interior_faces_once OR assemble_own_interior_faces_both."));

  Assert(
    (flags & (dealii::MeshWorker::assemble_ghost_faces_once | dealii::MeshWorker::assemble_ghost_faces_both)) !=
      (dealii::MeshWorker::assemble_ghost_faces_once | dealii::MeshWorker::assemble_ghost_faces_both),
    dealii::ExcMessage(
      "You can only specify assemble_ghost_faces_once OR assemble_ghost_faces_both."));

  Assert(
    !(flags & dealii::MeshWorker::cells_after_faces) ||
      (flags & (dealii::MeshWorker::assemble_own_cells | dealii::MeshWorker::assemble_ghost_cells)),
    dealii::ExcMessage(
      "The option cells_after_faces only makes sense if you assemble on cells."));

  Assert((!face_worker) == !(flags & dealii::MeshWorker::work_on_faces),
         dealii::ExcMessage(
           "If you specify a face_worker, assemble_face_* needs to be set."));

  Assert(
    (!boundary_worker) == !(flags & dealii::MeshWorker::assemble_boundary_faces),
    dealii::ExcMessage(
  ...
  auto cell_action = [&](const CellIteratorBaseType &cell,
    ...
    dealii::types::subdomain_id current_subdomain_id =
    ...
    if ((!ignore_subdomain) &&
        (current_subdomain_id == dealii::numbers::artificial_subdomain_id))
      return;
           ...
           ++face_no)
        {
          if (cell->at_boundary(face_no) &&
              !cell->has_periodic_neighbor(face_no))
            {
              ...
              if ((flags & dealii::MeshWorker::assemble_boundary_faces) && own_cell)
                boundary_worker(cell, face_no, scratch, copy);
            }
          else
            {
              // interior face, potentially assemble
              dealii::TriaIterator<typename CellIteratorBaseType::AccessorType>
                neighbor = cell->neighbor_or_periodic_neighbor(face_no);

              dealii::types::subdomain_id neighbor_subdomain_id =
              ...
              if (own_cell && own_neighbor &&
                  !(flags & (dealii::MeshWorker::assemble_own_interior_faces_both |
                          dealii::MeshWorker::assemble_own_interior_faces_once)))
                continue;

              // skip face to ghost
              if (own_cell != own_neighbor &&
                  !(flags &
                    (dealii::MeshWorker::assemble_ghost_faces_both | dealii::MeshWorker::assemble_ghost_faces_once)))
                continue;
              ...
              if ((!periodic_neighbor &&
                ...
                {
                  Assert(!cell->has_children(), dealii::ExcInternalError());
                  Assert(!neighbor->has_children(), dealii::ExcInternalError());

                  // skip if only one processor needs to assemble the face
                  // to a ghost cell and the fine cell is not ours.
                  if (!own_cell && (flags & dealii::MeshWorker::assemble_ghost_faces_once))
                    continue;
                  ...
                  if (flags & dealii::MeshWorker::assemble_own_interior_faces_both)
                    {
              ...
              else
                {
                  ...
                  // internal face.
                  if (efi_internal::is_active_iterator(cell) &&
                  ...
                  // dealii::MeshWorker::AssembleFlags says otherwise). Here, we rely on cell
                  // comparison that will look at cell->index().
                  if (own_cell && own_neighbor &&
                      (flags & dealii::MeshWorker::assemble_own_interior_faces_once) &&
                      (neighbor < cell))
                    continue;
                  ...
                  // assemble the face.
                  if (own_cell && !own_neighbor &&
                      (flags & dealii::MeshWorker::assemble_ghost_faces_once) &&
...
void
mesh_loop(
  dealii::IteratorRange<CellIteratorType> iterator_range,
  const typename dealii::identity<std::function<
    void(const CellIteratorBaseType &, ScratchData &, CopyData &)>>::type
    &cell_worker,
  const typename dealii::identity<std::function<void(const CopyData &)>>::type
  ...
  const unsigned int chunk_size   = 8)
{
  ...
  mesh_loop<typename dealii::IteratorRange<CellIteratorType>::dealii::IteratorOverIterators,
            ScratchData,
            CopyData,
...
template <class CellIteratorType,
          ...
          class CellIteratorBaseType =
            typename efi_internal::CellIteratorBaseType<CellIteratorType>::type>
void
mesh_loop(dealii::IteratorRange<CellIteratorType> iterator_range,
          MainClass &                     main_class,
...
#endif /* SRC_EFI_INCLUDE_EFI_BASE_BUGFIX_MESH_LOOP_H_ */
