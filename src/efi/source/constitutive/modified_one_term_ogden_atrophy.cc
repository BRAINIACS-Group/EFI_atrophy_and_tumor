...
get_data_interpretation () const
{
    ...
    // NEW: mu field (voxel-wise)
    data_interpretation.push_back (
            create_data_interpretation<Tensor<0,dim,scalar_type>>("mu",position));
    position += data_interpretation.back().n_components();

    // NEW: alpha field (voxel-wise)
    data_interpretation.push_back (
            create_data_interpretation<Tensor<0,dim,scalar_type>>("alpha",position));
    position += data_interpretation.back().n_components();
                       ...
                       const dealii::GeneralDataStorage *) const
{
    ...
    for (unsigned int q=0; q<input_data.solution_values.size(); ++q)
    {
        double *computed_quantities_ptr =
            std::addressof(computed_quantities[q][0]) + pos + 2;
        TensorShape<0,dim,double> mu_field (computed_quantities_ptr);
        mu_field = this->mu;
    }
    ...
    // alpha -> pos+3
    for (unsigned int q=0; q<input_data.solution_values.size(); ++q) {
        double *p = std::addressof(computed_quantities[q][0]) + pos + 3;
        TensorShape<0,dim,double> alpha_field(p);
        alpha_field = this->alpha;
    }
